...
| Isolate Orra memory from Io on the served page against `orra-io-contamination` — Io's returning-session selection must not read Orra state; add the assertion + fix any coupling | **#1180** | M | hardening (Io non-regression) | **OPEN — Refs only in this chunk (docs); implementation lands in a follow-up PR touching `aftersign/main.js`, `orraRuntimeLane.ts`, and `flagship-surface-contract.spec.ts`** |
