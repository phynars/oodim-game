...
- `read_issue(1827)`: closed September 23; focused rendered divergence and determinism scope. Its proposed `aftersign/e2e/two-save-tappable-divergence.spec.ts` path is an issue-described new file; a file at that path is now present on main, not verified as executed here. Closed status is not proof its acceptance ran.
...
- `read(aftersign/e2e/m-loop-e1-two-round-playtest.spec.ts)` — two cases. Case 1 plays a single fresh slot from `packet-offered` through delivery, recognition, return-tone, next-job, and back to a second `packet-offered`; it asserts the visible `#job-offer-*` id set changes from `[job-offer-job-safe-delivery]` to the `[job-offer-job-night-transfer, job-offer-job-signed-receipt]` set. This is a within-record two-round action-set divergence proof on rendered controls via `.tap()`, with `window.__game` read only for readiness and a fingerprint snapshot. It does not tap round 2's new offers, and it exercises one memory record, not two. Case 2, explicitly associated with #1819, opens two `?slot=` records, plays record B through one loop to its next-offer beat, and compares record B's rendered offer ids against record A's first-visit ids. It does not play record B's round 2 or tap record B's divergent controls. Both cases assert selected visible transitions (beat gates, recognition text, offer id sets) but do not assert every visible dialogue change enumerated in the M-LOOP bar.
...
3. `m-loop-e1-two-round-playtest.spec.ts` case 1 does play a within-record two-round action-set divergence; its #1819-associated case 2 stops at record B's next offer without playing record B's round 2 or tapping its divergent controls. The remaining acceptance gap is per-record two-round completion across two divergent records, not proof of any played round at all.
4. Exact next action: read `aftersign/e2e/m-loop-two-round-divergence.playtest.spec.ts`, `m-loop-two-rounds-phone-playtest.spec.ts`, and `m-loop-divergence.playtest.spec.ts` in full to determine whether any sibling spec already closes the two-record two-round gap before filing repair. Do not assume from filename.
5. Check two rounds for each durable record, every visible dialogue change (not only beat-gate and recognition-line samples), selectable divergence in record B, and absence of action-causing harness hooks.
6. #1827 is closed September 23; inspect its landed focused spec only as needed to avoid overlapping work and verify determinism coverage.
7. Retrieve deployed non-skipped execution artifacts and human replay evidence. Neither is verified; neither is declared absent.
8. Read #1950/#1920 bodies before final drift classification; #1825 remains operator work.
9. File only demonstrated bounded S/M gaps, integration first, with canonical metadata and current days remaining. Reuse existing identities/evidence. If the demonstrated gap is per-record round 2 in the #1819 case, the residual is a bounded S/M edit to that existing spec — not a new integration story.
10. Finish story-map sizing and artifact links. Do not mark M2 done from issue closure, merged components, tray attributes, source-pattern guards, or a single within-record two-round proof alone.
