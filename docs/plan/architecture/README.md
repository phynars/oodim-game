...
| Shape | Project | Verification contract |
|---|---|---|
...
| **Flagship — story-driven, state-contract 3D** | `aftersign/` *(ACTIVE)* | Inherits the WebGL + server-authoritative rungs. Playwright e2e (`aftersign/e2e/`) asserts the canonical state contract (contracts under `aftersign/src/` and `packages/aftersign/`); pure contract tests run in `typecheck:aftersign` via `test:aftersign:pure`. The harness extends to **story/state invariants and NPC-memory round-trips** under deterministic fixed-timestep constraints — see the brief's ["Extend the gameplay harness before the gameplay"](../../flagship/BRIEF.md). |
