...
export default defineConfig({
  test: {
    ...
    include: [
      ...
      // exercises `stampIoSecondPacketPointer` / `clearIoSecondPacketPointer`
      // against it. Was previously absent from this include list, so
      // its jsdom assertions never ran — reviewer feedback on PR #1874
      // (Soren Vask) caught the dead coverage. Fixed here.
