...
async function performPacketGesture(
  ...
  // OPENED — a hold with a 12px mid-hold pull, dispatched against the
  // rendered `#packetButton` from inside `page.evaluate`. We deliberately
  // KEEP the sleep inside `page.evaluate` (`setTimeout` in browser context)
  // rather than lifting it to `page.waitForTimeout` — the no-wall-clock-
  // waits guard scans Playwright test files for `waitForTimeout(` at
  // source level (e2e-shared/no-wall-clock-waits/check.mjs), and any real
  // call there fails CI unless annotated. The gesture still crosses the
  // PacketIntentController's OPEN thresholds (pull=12px sits inside
  // (OPEN_PULL_MIN_PX=10, DRIFT_CANCEL_PX=14]; hold=900ms clears the OPEN
  // window), so `commitPacketOutcome(OPENED)` fires exactly as before.
