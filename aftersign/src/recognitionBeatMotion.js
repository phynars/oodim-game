...
export function playRecognitionBeat({ dialogueEl, signEl, reducedMotion = false }) {
  ...
  // Respect reduced-motion before changing any visual state.
  if (reducedMotion || typeof window === "undefined" || typeof window.setTimeout !== "function") {
    return () => {};
  }
  ...
  const settle = window.setTimeout(() => {
    dialogueEl.style.transition = "transform 180ms cubic-bezier(0.16, 1, 0.3, 1)";
