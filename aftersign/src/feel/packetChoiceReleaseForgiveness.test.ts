...
// still imports `runPacketChoiceReleaseForgivenessChecks` directly from
// `../src/feel/packetChoiceReleaseForgiveness` — that path resolves to the
// `.ts` module above, not this shim. This file exists so
// `typecheck:aftersign` picks up the module (aftersign/tsconfig.json
// deliberately does NOT exclude `**/*.test.ts` — see its header) and any
// future pure-runner entry has a stable import target.
