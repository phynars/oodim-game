...
// runRouteChoicePressFeedbackChecks itself, per its registration
// checklist item 2. A top-level call here would double-run the bundle
// if a Playwright spec ever imports it. See
// `aftersign/src/routeRiskFeel.test.ts` and
// `aftersign/src/targetLossFeedback.test.ts` for the canonical shape.
export { runRouteChoicePressFeedbackChecks } from "./routeChoicePressFeedback.ts";
