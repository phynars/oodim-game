// AFTERSIGN packet intent contract — deliberate-hold guard for the first
// slice choice (open the sealed blue packet vs. preserve it).
//
// Feel goal: a stray tap must never break the seal. Tap = inspect only;
// opening requires a continuous, focused hold that crosses the threshold.
// Losing inspection focus (background frame, tab switch) cancels all
// accumulated hold progress.
//
// Harness convention (see aftersign/README.md): assertion helpers are plain
// TypeScript that throw on failure; the runner entry is the exported
// `runPacketIntentChecks()` — no vitest.
