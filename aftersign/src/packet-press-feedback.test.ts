...
// `runPacketPressFeedbackChecks` itself, per its registration
// checklist item 2. A top-level call here would double-run the bundle
// if a Playwright spec ever imports it. See
// `aftersign/src/routeChoicePressFeedback.test.ts` and
// `aftersign/src/targetLossFeedback.test.ts` for the canonical shape.
export { runPacketPressFeedbackChecks } from "./packet-press-feedback.ts";
